...
export class PlaylistPlug extends BasePlug {
  public static readonly plugName: string = "playlist";

  public wire(): void {
    this.ctl.state.currentPlaylistIndex = 0;
    ...
    this.ctl.config.on("playlist", this.handlePlaylistChange, { signal: this.signal, immediate: true });
  }
  ...
  protected handlePlaylistChange({ root }: Event<VideoBuild, "playlist">): void {
    ...
    this.ctl.state.currentPlaylistIndex = v ? list?.indexOf(v) : 0;
    if (v) this.applyItem(v, false);
    else this.movePlaylistTo(this.ctl.state.currentPlaylistIndex);
  }

  public movePlaylistTo(index: number, shouldPlay?: boolean): void {
    if (!this.ctl.config.playlist) return;
    this.ctl.state.currentPlaylistIndex = index;
  ...
  public previousVideo(): void {
    ...
    else if (this.ctl.config.playlist && this.ctl.state.currentPlaylistIndex > 0) this.movePlaylistTo(this.ctl.state.currentPlaylistIndex - 1, true);
  }

  public nextVideo(): void {
    if (!this.ctl.config.playlist) return;
    if (this.ctl.state.currentPlaylistIndex < this.ctl.config.playlist.length - 1) this.movePlaylistTo(this.ctl.state.currentPlaylistIndex + 1, true);
  }
}
