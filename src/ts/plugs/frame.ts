...
export class FramePlug extends BasePlug<Frame> {
  ...
  public async getFrame(display = "", time = this.ctlr.media.state.currentTime, raw = false, min = 0, video = this.ctlr.pseudoVideo): Promise<{ canvas: HTMLCanvasElement; context: CanvasRenderingContext2D } | { blob: Blob | false; url: string | false }> {
    ...
    this.exportContext.filter = this.ctlr.config.settings.css.filter as string;
  ...
  public async captureFrame(display = "", time = this.ctlr.media.state.currentTime): Promise<void> {
    ...
    const toast = this.ctlr.getPlug<ToastsPlug>("toasts")?.toast,
      ...
      frameToastId = toast?.loading(`Capturing ${fTxt}...`, { delay: parseCSSTime(this.ctlr.config.settings.css.notifiersAnimationTime), image: window.TMG_VIDEO_ALT_IMG_SRC, tag: `tmg-${this.ctlr.config.media.title ?? "Video"}fcpa${tTxt}${display}` }) as string,
      frame = (await this.getFrame(display, time, false, 0, this.ctlr.media.element)) as { blob: Blob | false; url: string | false },
  ...
  public async findGoodTime({ time: t = this.ctlr.media.state.currentTime, secondsLimit: s = 25, saturation: sat = 12, brightness: bri = 40 } = {}): Promise<number | null> {
  ...
  public async getMainColor(time?: number, poster = this.ctlr.media.element.poster, config = {}): Promise<string | [number, number, number] | null> {
  ...
  public moveFrame(dir: "forwards" | "backwards" = "forwards"): void {
    this.ctlr.media.state.paused && this.ctlr.throttle("frameStepping", () => (this.ctlr.media.intent.currentTime = clamp(0, Math.round(this.ctlr.media.state.currentTime * this.config.fps) + (dir === "backwards" ? -1 : 1), Math.floor(this.ctlr.media.status.duration * this.config.fps)) / this.config.fps), Math.round(1000 / this.config.fps));
  }
}
