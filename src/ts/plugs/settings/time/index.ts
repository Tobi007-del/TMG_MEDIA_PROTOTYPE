import { BasePlug, GesturePlug, OverlayPlug, type KeysPlug, type KeyMod, CTime, TimeState, TIME_BUILD } from "../..";
...
export class TimePlug extends BasePlug<CTime, TimeState> {
  ...
  public skip(duration: number): void {
    const overlay = this.ctlr.plug<OverlayPlug>("overlay"),
      notifier = duration > 0 ? this.ctlr.queryDOM(".tmg-media-fwd-notifier") : this.ctlr.queryDOM(".tmg-media-bwd-notifier");
    ...
    if (mdle?.state.skipPersist) {
      if (this.currentSkipNotifier && notifier !== this.currentSkipNotifier) {
        this.skipDuration = 0;
        this.currentSkipNotifier.classList.remove("tmg-media-control-persist");
      }
      overlay?.show();
    ...
    } else this.currentSkipNotifier?.classList.remove("tmg-media-control-persist");
