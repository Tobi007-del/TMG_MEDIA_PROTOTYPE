...
export class Controller {
  ...
  constructor(medium: HTMLVideoElement, build: CtlrConfig) {
    ...
    this.media = reactive<CtlrMedia>({ element: inert(medium), tech: inert({} as BaseTech), intent: volatile(intent(defs.intent)), state: state(defs.state), status: state(defs.status), settings: state(defs.settings) });
  ...
  private boot() {
    this.connectPlugs();
    this.wireTechHandler();
    this.wireStateHandler();
  ...
  protected wireTechHandler() {
    ...
    this.media.on("state.sources", () => this.handleTech("intent"), { capture: true, signal: this.signal }); // just in case :)
    this.media.on("settings.srcObject", () => this.handleTech(), { capture: true, signal: this.signal });
  }
  protected handleTech(pref: "state" | "intent" = "intent") {
