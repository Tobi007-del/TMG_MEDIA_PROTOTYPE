...
let _audioLimiter: DynamicsCompressorNode | null = null;
let _isDocTransient = false;
...
export function startAudioManager() {
  if (!_audioContext && _isDocTransient && typeof window !== "undefined") {
    _audioContext = new (window.AudioContext || (window as any).webkitAudioContext)() as AudioContext;
    const L = (_audioLimiter = _audioContext!.createDynamicsCompressor());
    ((L.threshold.value = -1.0), (L.knee.value = 0.0), (L.ratio.value = 20), (L.attack.value = 0.001), (L.release.value = 0.05));
    Controllers.forEach((c) => (c.runtime.audioContextReady = true));
  } else if (_audioContext?.state === "suspended") _audioContext.resume();
}

export function connectMediaToAudioManager(medium: HTMLMediaElement) {
  if (!_audioContext) return "unavailable";
  medium.mediaElementSourceNode ??= _audioContext.createMediaElementSource(medium);
  medium._tmgGainNode ??= _audioContext.createGain();
  medium._tmgDynamicsCompressorNode ??= _audioContext.createDynamicsCompressor();
  ...
  medium._tmgGainNode.connect(_audioLimiter!);
  _audioLimiter!.connect(_audioContext!.destination);
}
...
export function init() {
  mountMedia();
  ["pointerdown", "keydown"].forEach((e) => document?.addEventListener(e, () => ((_isDocTransient = true), startAudioManager()), true));
  ...
  window?.addEventListener("resize", () => Controllers.forEach((c) => (c.runtime.dimensions.window = { width: window.innerWidth, height: window.innerHeight })));
  screen?.orientation.addEventListener("change", (e) => Controllers.forEach((c) => (c.runtime.screenOrientation = e?.target as ScreenOrientation)));
  document?.addEventListener("visibilitychange", () => Controllers.forEach((c) => (c.runtime.docVisibilityState = document.visibilityState)));
  ["fullscreenchange", "webkitfullscreenchange", "mozfullscreenchange", "msfullscreenchange"].forEach((e) => document?.addEventListener(e, () => Controllers.forEach((c) => (c.runtime.docInFullscreen = queryFullscreen()))));
}
