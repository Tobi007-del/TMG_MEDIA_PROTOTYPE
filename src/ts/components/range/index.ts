import { BaseComponent, RangeConfig, RangeState } from "..";
...
export class RangeSlider<Config extends RangeConfig = RangeConfig, State extends RangeState = RangeState> extends BaseComponent<Reactive<Config>, State> {
  ...
  constructor(ctlr: Controller, config: Partial<Config> = {}) {
    const defaults = { label: "Range", min: 0, max: 100, value: 0, previewValue: 50, step: 1, scrub: { sync: true, relative: true, cancel: { delta: 15, timeout: 2000 }, wheel: { disabled: false, axisRatio: 6 } } };
  ...
  public override create(): HTMLElement {
    ...
    const container = createEl("div", { className: "tmg-media-range-container", tabIndex: 0, role: "slider" });
    ...
    container.append(this.barsWrapper, this.thumbEl);
  ...
  public override wire(): void {
    ...
    // ------ Listeners
    this.config.on("label", ({ value }) => (this.el.ariaLabel = value!), { signal: this.signal, immediate: true });
    this.config.on("min", ({ value }) => (this.el.ariaValueMin = String(value!)), { signal: this.signal, immediate: true });
    this.config.on("max", ({ value }) => (this.el.ariaValueMax = String(value!)), { signal: this.signal, immediate: true });
  ...
  protected handlePointerDown(e: PointerEvent): void {
    ...
    this.handleInput(e);
    this.el.addEventListener("pointermove", this.handleInput, { signal: this.signal });
    this.el.addEventListener("pointerup", this.stopScrubbing, { signal: this.signal });
  }
  ...
  protected stopScrubbing(): void {
    ...
    this.el.removeEventListener("pointermove", this.handleInput);
    this.el.removeEventListener("pointerup", this.stopScrubbing);
  }
    ...
    this.ctlr.throttle(
      ...
      () => {
        ...
        if (this.state.scrubbing) {
          !this.config.scrub.sync ? this.syncThumbPos(pos) : this.seek(value);
  ...
  public syncThumbPos(pos: number): void {
    this.thumbEl.style.cssText = `${this.isVertical ? "inset-block-end" : "inset-inline-start"}: ${pos * 100}%`;
    // this.thumbEl.style.transform = this.isVertical ? `translateY(-${pos * 100}%)` : `translateX(${pos * 100}%)`;
  }
