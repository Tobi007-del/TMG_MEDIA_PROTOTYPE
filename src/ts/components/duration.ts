...
export class Duration extends BaseComponent<DurationConfig, ComponentState, HTMLButtonElement> {
  ...
  public wire(): void {
    ...
    this.ctl.media.on("status.duration", this.updateUI, { signal: this.signal, immediate: true });
    this.ctl.config.on("settings.time.format", this.updateARIA, { signal: this.signal, immediate: true });
  }

  protected updateUI(): void {
    this.element.textContent = this.plug?.toTimeText(this.ctl.media.status.duration) ?? "--:--";
  }
  protected updateARIA() {
    this.state.label = "Switch time format";
    this.state.cmd = formatKeyForDisplay(this.ctl.config.settings.time.format);
