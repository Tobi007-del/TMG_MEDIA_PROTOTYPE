import { RangeSlider, TimelineConfig, type RangeState } from "..";
...
export class Timeline extends RangeSlider<TimelineConfig> {
  ...
  constructor(ctlr: Controller, config?: Partial<TimelineConfig>) {
    super(ctlr, { label: "Media timeline", ...config });
  }

  public override create() {
    // Variables Assignments
    const container = super.create();
    container.dataset.controlId = this.name;
    this.timeline = createEl("div", { className: "tmg-media-timeline" });
    this.bufferedBar = createEl("div", { className: "tmg-media-range-bar tmg-media-seek-bar tmg-media-buffered-seek-bar" });
    this.previewBar = createEl("div", { className: "tmg-media-range-bar tmg-media-seek-bar tmg-media-preview-seek-bar" });
    ...
    // DOM Injection
    container.classList.add("tmg-media-timeline-container"), this.barsWrapper.classList.add("tmg-media-seek-bars-wrapper"), this.baseBar.classList.add("tmg-media-seek-base-bar"), this.valueBar.classList.add("tmg-media-seek-value-bar"), this.thumbEl.classList.add("tmg-media-seek-thumb");
    ...
    return container; // parent handled element assignment
  }
  ...
  protected handleScrubbing({ value }: REvent<RangeState, "scrubbing">): void {
    if (!value) {
      ...
      // this.ctlr.DOM.scrubNotifier?.classList.remove("tmg-media-control-active");
    } else {
      ...
      this.scrubbingId = requestAnimationFrame(() => {
        ...
        // IS_MOBILE && this.ctlr.DOM.scrubNotifier?.classList.add("tmg-media-control-active");
      }, this.signal);
    }
