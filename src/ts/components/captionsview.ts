...
export class CaptionsView extends BaseComponent<CaptionsViewConfig, ComponentState, HTMLDivElement> {
  ...
  public preview(preview: CueLike | string = `${capitalize(this.media.container.dataset.trackKind || "captions")} look like this`, flush = this.element.textContent.replace(/\s/g, "") === this.lastPreview?.replace(/\s/g, "")): void {
    ...
    this.previewTimeoutId = setTimeout((flush = this.element.textContent.replace(/\s/g, "") === text.replace(/\s/g, "")) => (this.media.container.classList.remove("tmg-media-captions-preview"), flush && (this.element.innerHTML = "")), this.ctlr.settings.captions.previewTimeout, this.signal);
    this.lastPreview = text;
  }

  public render(cue: CueLike | null): void {
    const existing = this.element.querySelector<HTMLElement>(".tmg-media-captions-wrapper");
    ...
    lines.forEach((p) => formatVttLine(p, Math.floor(vCWidth / this.charW)).forEach((l) => wrapper.append(createEl("div", { className: "tmg-media-captions-line" }, cue.id ? { part: "cue", id: cue.id } : { part: "cue" }, allowOverride && cue.align ? { textAlign: cue.align } : undefined)!.appendChild(createEl("span", { className: "tmg-media-captions-text", innerHTML: parseVttText(l) })!).parentElement!)));
    !existing && this.element.append(wrapper);
    const { offsetWidth: cWidth, offsetHeight: cHeight } = this.element;
    this.isMain ? (this.ctlr.settings.css.currentCaptionsContainerHeight = `${cHeight}px`) : this.element.style.setProperty("--tmg-media-current-captions-container-height", `${cHeight}px`);
    this.isMain ? (this.ctlr.settings.css.currentCaptionsContainerWidth = `${cWidth}px`) : this.element.style.setProperty("--tmg-media-current-captions-container-width", `${cWidth}px`);
    if (allowOverride) {
      if (cue.region) {
        ...
        if (isDef(vpAnX)) this.element.style.setProperty("--tmg-media-current-captions-x", `${vpAnX}%`);
        if (isDef(vpAnY)) this.element.style.setProperty("--tmg-media-current-captions-y", `${100 - Number(vpAnY)}%`);
        ...
        if (isDef(cue.position) && cue.position !== "auto") {
          ...
          this.element.style.setProperty("--tmg-media-current-captions-x", `calc(${cue.position}% + ${posOffset}% + ${elHalfWPct}%)`);
        }
        if (isDef(cue.line) && cue.line !== "auto") {
          ...
          this.element.style.setProperty("--tmg-media-current-captions-y", `calc(${bottomVal}% + ${lineOffset}% + ${elHalfHPct}%)`);
        }
  ...
  protected handleDragging(e: PointerEvent): void {
    ...
    this.ctlr.RAFLoop("captionsDragging", () => {
      ...
      this.isMain ? (this.ctlr.settings.css.currentCaptionsX = `${(posX / ww) * 100}%`) : this.element.style.setProperty("--tmg-media-current-captions-x", `${(posX / ww) * 100}%`);
      this.isMain ? (this.ctlr.settings.css.currentCaptionsY = `${(posY / hh) * 100}%`) : this.element.style.setProperty("--tmg-media-current-captions-y", `${(posY / hh) * 100}%`);
    });
  }
